...
import { Card, SimpleGrid, Box, Text, Image, ActionIcon, Loader, Button, Overlay, AspectRatio, Group } from '@mantine/core';
  ...
  return (
    <>
      <Card ref={setNodeRef} style={style} shadow="sm" radius="md" withBorder padding={0}>
        {/* Cabecera Superior */}
        <Group justify="space-between" align="center" px="md" py="xs" bg="gray.0" style={{ borderBottom: '1px solid var(--mantine-color-gray-2)' }}>
          {/* Extremo Izquierdo: Drag Handle */}
          <Box style={{ cursor: isDragging ? 'grabbing' : 'grab', display: 'flex', alignItems: 'center' }} {...attributes} {...listeners}>
            <IconGripVertical size={20} color="var(--mantine-color-gray-5)" />
          </Box>

          {/* Centro: Título */}
          <Text fw={700} ta="center" style={{ letterSpacing: 1 }}>
            PAR #{par.orden}
          </Text>
